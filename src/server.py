...
qwen_client = MockQwenClient() if not QWEN_AVAILABLE else None
orchestrator = None
...
async def get_orchestrator():
    ...
    if orchestrator is None:
        if QWEN_AVAILABLE:
            orchestrator = AuditOrchestrator(Generation, memory_store)
        else:
            orchestrator = AuditOrchestrator(MockQwenClient(), memory_store)
    return orchestrator
