...
class MockQwenClient:
    ...
    def __init__(self):
        self._chat_ns = _ChatNamespace(self)

    @property
    def chat(self):  # type: ignore
        return self._chat_ns

    async def _raw_chat(self, messages=None, temperature=0.1, max_tokens=4000, **kwargs):
        """Internal chat used by _Completions adapter"""
        mock_content = json.dumps({
            "vulnerabilities": [],
            "gas_issues": [],
            "architecture_review": "Mock analysis - set DASHSCOPE_API_KEY for real results",
            "risk_score": 0,
            "confidence": "low"
        })
        return {
            "choices": [{"message": {"content": mock_content, "role": "assistant"}}],
    ...
    def get_usage_stats(self):
        return {"model": "mock", "api_key_set": False, "dashscope_available": False}
...
class _Completions:
    """Adapts QwenClient.chat() to the OpenAI-style .chat.completions.create() interface"""
    def __init__(self, client):
        self._client = client

    async def create(self, model=None, messages=None, temperature=0.1, max_tokens=4000, **kwargs):
        # QwenClient uses .chat(), MockQwenClient uses ._raw_chat()
        _fn = getattr(self._client, "_raw_chat", None) or getattr(self._client, "chat")
        result = await _fn(
...
# Patch QwenClient and MockQwenClient with .chat.completions namespace
_original_qwen_init = QwenClient.__init__

def _patched_qwen_init(self, *args, **kwargs):
    _original_qwen_init(self, *args, **kwargs)
    self.chat = _ChatNamespace(self)  # type: ignore

QwenClient.__init__ = _patched_qwen_init  # type: ignore
